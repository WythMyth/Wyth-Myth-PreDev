...
class UserRegistrationForm(forms.ModelForm):
    ...
    is_us_based = forms.TypedChoiceField(
        label="Is your Home Address U.S. Based?",
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Conditional address fields (required logic handled in clean)
        for field_name in [
            "city",
            "state",
            "zip_code",
    ...
    def clean(self):
        ...
        is_us_based = cleaned_data.get("is_us_based")
        ...
        if not birth_date:
            ...
            try:
                ...
                if day > monthrange(year, month)[1]:
                    self.add_error(
                        "birth_date", "Invalid date for the selected month and year."
                    )
        ...
        if is_us_based is None:
            is_us_based = str(self.data.get("is_us_based", "")).lower() != "false"
            cleaned_data["is_us_based"] = is_us_based

        def require_field(field_name, message):
...
class UserUpdateForm(forms.ModelForm):
    ...
    beneficiary_percentages = forms.CharField(
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.data.get("birth_year") and self.data.get("birth_month"):
            try:
                ...
                self.fields["birth_date"].choices += [
                    (str(d), str(d)) for d in range(1, day_count + 1)
                ]
            except:
                pass
        elif self.instance.birth_year and self.instance.birth_month:
            day_count = monthrange(self.instance.birth_year, self.instance.birth_month)[
                1
            ]
            self.fields["birth_date"].choices += [
                (str(d), str(d)) for d in range(1, day_count + 1)
            ...
            for name, percentage in zip(names, percentages):
                ...
                if name.strip() and percentage.strip():
                    try:
                        ...
                        if pct < 0:
                            self.add_error(
                                None, f"Percentage cannot be negative: {percentage}"
                            )
                        else:
                            beneficiaries.append(
                                {"name": name.strip(), "percentage": pct}
                            )
                    except ValueError:
...
class PropertyForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "acquisition_cost": forms.NumberInput(
                attrs={"readonly": "readonly", "class": "bg-gray-100"}
            ),
        }
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["description"].label = "Description of the property"
        self.fields["estimated_price"].label = (
            "Original listing price [Zillow/Redfine] ($)"
        )
        ...
        self.fields["neighborhood_Demographic_Profile"].label = (
        ...
        self.fields["contributors"].label_from_instance = (
            lambda obj: f"{obj.get_full_name()} (Balance: ${obj.balance:,.2f})"
        )
...
class OfficeExpensePaymentForm(forms.ModelForm):
    ...
    user = forms.ModelChoiceField(
        queryset=User.objects.filter(is_active=True, investor=True).order_by(
            "first_name"
        ),
        required=True,
        label="User",
    ...
    class Meta:
        ...
        labels = {"user": "User", "receive_type": "Receive Type", "receipt": "Receipt"}
