...
class Property(models.Model):
    ...
    property_type = models.CharField(null=True, blank=True, max_length=20, choices=TYPE_CHOICES, default='single_family')
    exterior_feature = models.CharField(max_length=20, null=True, blank=True, choices=EXTERIOR_CHOICES, default='brick')
    neighborhood_Demographic_Profile = models.CharField(max_length=30, null=True, blank=True, choices=NEIGHBORHOOD_CHOICES, default='White (Non-Hispanic)')
    ...
    def distribute_sale_proceeds(self, force=False):
        """
        ...
        IMPORTANT FIXES:
        - Balance update is done ONCE PER USER, not per contribution row
        - Re-distribution is blocked when is_contribution_locked=True unless force=True
        """
        ...
        if self.is_contribution_locked and not force:
            print("⚠️ Distribution already completed for this property. Skipping duplicate run.")
            return False

        contributions = self.property_contributions.select_related("user", "user__user_group").all().order_by(
            "investment_date", "investment_sequence", "id"
        )
        ...
        if total_profit <= Decimal("0"):
            ...
            with transaction.atomic():
                ...
                for contrib in contributions:
                    ...
                    user_totals[user.id]["user"] = user
                    user_totals[user.id]["return_amount"] += refund_amount

                print("\n💰 UPDATING USER BALANCES:")
                print("-" * 100)

                for _, data in user_totals.items():
                    user = data["user"]
                    before_balance = Decimal(str(user.balance or 0))
                    total_return = data["return_amount"]

                    user.balance = (before_balance + total_return).quantize(
                        Decimal("0.000001"), rounding=ROUND_HALF_UP
                    )
                    user.save(update_fields=["balance"])

                    print(f"  {user.get_full_name():20} | Before: ${before_balance:10.2f}")
                    print(f"  {'':20} | Return: ${total_return:10.2f}")
            ...
            print("✅ Investment returns completed!")
            print("=" * 100)
            return True
        ...
        contributions_list = list(
            self.property_contributions.select_related("user", "user__user_group")
            .all()
            .order_by("investment_date", "investment_sequence", "id")
        )

        total_weight = sum(Decimal(str(c.profit_weight or 0)) for c in contributions_list)

        print(f"  Total Weight: {total_weight:.6f}")

        if total_weight <= Decimal("0"):
            print("❌ Total weight is zero. Cannot distribute profit.")
            return False
        ...
        with transaction.atomic():
            ...
            print("-" * 120)
            print(
                f"{'Name':20} | {'Group':15} | {'Seq':3} | "
                f"{'Investment':>12} | {'Weight':>10} | {'Profit':>12} | "
                f"{'Deduct%':>8} | {'Deduction':>11} | {'Final':>12}"
            )
            print("-" * 120)
            ...
            for contrib in contributions_list:
                ...
                gross_profit = (
                    total_profit * Decimal(str(contrib.profit_weight or 0)) / total_weight
                ).quantize(
                    Decimal("0.01"),
                    rounding=ROUND_HALF_UP
                )
            ...
            # IMPORTANT FIX: aggregate per user
            user_totals = defaultdict(lambda: {
                "user": None,
                "return_amount": Decimal("0.00"),
                "profit": Decimal("0.00"),
            ...
            for contrib in contributions_list:
                user = contrib.user
                user_totals[user.id]["user"] = user
                user_totals[user.id]["return_amount"] += Decimal(str(contrib.contribution or 0))
                user_totals[user.id]["profit"] += Decimal(str(contrib.profit or 0))
                user_totals[user.id]["deduction"] += Decimal(str(contrib.deduction or 0))
                user_totals[user.id]["final_profit"] += Decimal(str(contrib.final_profit or 0))
            ...
            for _, data in user_totals.items():
                ...
                before_balance = Decimal(str(user.balance or 0))
                total_return = data["return_amount"]
                total_profit_user = data["profit"]
                total_deduction_user = data["deduction"]
                total_final_profit_user = data["final_profit"]

                user.balance = (
                    before_balance + total_return + total_final_profit_user
                ).quantize(
                    Decimal("0.000001"),
                    rounding=ROUND_HALF_UP
                )
                ...
                print(f"  {user.get_full_name():20} | Before: ${before_balance:10.2f}")
                print(f"  {'':20} | Return: ${total_return:10.2f}")
                print(f"  {'':20} | Profit: ${total_profit_user:10.2f}")
                print(f"  {'':20} | Deduct: ${total_deduction_user:10.2f}")
                print(f"  {'':20} | Final:  ${total_final_profit_user:10.2f}")
                print(f"  {'':20} | After:  ${Decimal(str(user.balance or 0)):10.2f}\n")
            ...
            if total_deduction > 0:
                if office_manager:
                    office_manager.balance = (
                        Decimal(str(office_manager.balance or 0)) + total_deduction
                    ).quantize(Decimal("0.000001"), rounding=ROUND_HALF_UP)
                    office_manager.save(update_fields=["balance"])
                    print(f"  ✅ Added to Office Manager: {office_manager.get_full_name()}")
                else:
                    office_cost.balance = (
                        Decimal(str(office_cost.balance or 0)) + total_deduction
                    ).quantize(Decimal("0.000001"), rounding=ROUND_HALF_UP)
                    office_cost.save(update_fields=["balance"])
...
class RentalBill(models.Model):
    ...
    property = models.ForeignKey(
        "Property",
        on_delete=models.CASCADE,
        related_name="rental_bills",
        verbose_name="Property",
    )

    rent_month = models.PositiveSmallIntegerField(
        choices=MONTH_CHOICES,
        verbose_name="Rent Month", null=True, blank=True
    )

    rent_year = models.PositiveIntegerField(
        verbose_name="Rent Year", null=True, blank=True
    )

    bill_month = models.DateField(
        verbose_name="Bill Month",
        help_text="Auto-generated from rent month and year.", null=True, blank=True
    )
    ...
    approved_by = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="approved_rental_bills",
        verbose_name="Approved By",
    )
    ...
    finalized_by = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="finalized_rental_bills",
    ...
    def __str__(self):
        return f"{self.property.property_name} - {self.bill_month:%B %Y}"

    def clean(self):
        if self.property and self.property.status != "rented":
            raise ValidationError("Rental bill can be created only for rented property.")

        if self.rent_amount is not None and self.rent_amount < 0:
            raise ValidationError("Rent amount cannot be negative.")

        if self.rent_month and (self.rent_month < 1 or self.rent_month > 12):
            raise ValidationError("Invalid rent month.")

        if self.rent_year and self.rent_year < 1900:
            raise ValidationError("Invalid rent year.")

    def save(self, *args, **kwargs):
        if self.rent_month and self.rent_year:
            self.bill_month = date(self.rent_year, self.rent_month, 1)
        elif self.bill_month:
            self.bill_month = self.bill_month.replace(day=1)
            self.rent_month = self.bill_month.month
            self.rent_year = self.bill_month.year

        super().save(*args, **kwargs)

    @builtins.property
    def approved_expenses(self):
        return self.expenses.filter(status="approved")

    @builtins.property
    def total_expense(self):
        return self.approved_expenses.aggregate(
    ...
    def net_amount(self):
        return (
            Decimal(str(self.rent_amount or 0)) -
            Decimal(str(self.total_expense or 0))
        ).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)

    @builtins.property
    def is_profit(self):
        return self.net_amount > Decimal("0.00")
    ...
    @builtins.property
    def is_break_even(self):
        return self.net_amount == Decimal("0.00")

    def get_month_start_end(self):
        start = self.bill_month.replace(day=1)
    ...
    def approve(self, approved_by):
        if not approved_by.is_superuser:
            return False, "Only superuser can approve rental bill."

        if self.status != "draft":
            return False, "Only draft rental bill can be approved."

        self.status = "approved"
        self.approved_by = approved_by
        self.approved_at = timezone.now()
        self.save(update_fields=["status", "approved_by", "approved_at"])
        return True, "Rental bill approved successfully."

    def get_active_contributions(self):
        """
        Active Layer = contribution > 0.
        Non-invested rows should have contribution = 0.
        """
        month_start, month_end = self.get_month_start_end()

        return (
            self.property.property_contributions
            .select_related("user", "user__user_group")
            .filter(
                contribution__gt=0,
                investment_date__lte=month_end,
            )
            .order_by("investment_date", "investment_sequence", "id")
        )

    def get_month_days_for_contribution(self, contribution):
        """
        January = 31 days.
        Investment date Jan 22 => Jan 22 to Jan 31 = 10 days.
        """
        month_start, month_end = self.get_month_start_end()

        if contribution.investment_date:
            start_date = max(contribution.investment_date, month_start)
        else:
    ...
    def calculate_rental_weights(self):
        """
        base_weight = days_proportion × shares
        final_weight = base_weight × buyer_level_multiplier
        """
        contributions = list(self.get_active_contributions())
        ...
        for contrib in contributions:
            days = self.get_month_days_for_contribution(contrib)
            month_days_map[contrib.id] = days
        ...
        for contrib in contributions:
            days = month_days_map.get(contrib.id, 0)
            ...
            days_proportion = (
                Decimal(str(days)) / Decimal(str(max_days))
            ).quantize(Decimal("0.000001"), rounding=ROUND_HALF_UP)

            base_weight = (
                days_proportion * Decimal(str(contrib.shares or 0))
            ).quantize(Decimal("0.000001"), rounding=ROUND_HALF_UP)

            buyer_level = ""
            buyer_multiplier = Decimal("1.00")

            try:
                profit_dist = self.property.profit_distribution
                buyer_level = profit_dist.get_user_buyer_level(contrib.user) or ""
                buyer_multiplier = profit_dist.get_user_share_multiplier(contrib.user)
            except Exception:
                buyer_level = ""
                buyer_multiplier = Decimal("1.00")

            final_weight = (
                base_weight * Decimal(str(buyer_multiplier))
            ).quantize(Decimal("0.000001"), rounding=ROUND_HALF_UP)
            ...
            weighted_rows.append({
                ...
                "user": contrib.user,
                "month_days": days,
                "days_proportion": days_proportion,
                "shares": Decimal(str(contrib.shares or 0)),
    ...
    def preview_distribution(self):
        weighted_rows = self.calculate_rental_weights()
        net_amount = self.net_amount

        total_weight = sum(
            Decimal(str(row["final_weight"] or 0)) for row in weighted_rows
        ).quantize(Decimal("0.000001"), rounding=ROUND_HALF_UP)
        ...
        total_rows = len(weighted_rows)

        for index, row in enumerate(weighted_rows):
            user = row["user"]
            weight = Decimal(str(row["final_weight"] or 0))

            if index == total_rows - 1:
                raw_share = net_amount - distributed_total
            else:
                raw_share = (
                    net_amount * weight / total_weight
                ).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
                distributed_total += raw_share
            ...
            if net_amount > 0:
                gross_profit = raw_share
                deduction = (
                    gross_profit * deduction_percentage / Decimal("100")
                ).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
                final_profit = (
                    gross_profit - deduction
                ).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)

            elif net_amount < 0:
                loss_amount = abs(raw_share).quantize(
                    Decimal("0.01"),
                    rounding=ROUND_HALF_UP,
                )
    ...
    def finalize_distribution(self, finalized_by=None):
        """
        Only approved rental bill can be finalized.
        Finalize means balance update and distribution lock.
        """
        if not finalized_by or not finalized_by.is_superuser:
            return {
                "success": False,
                "message": "Only superuser can finalize rental bill.",
            }

        if self.status == "draft":
            return {
                "success": False,
                "message": "Rental bill must be approved before finalizing.",
            }

        if self.status == "finalized":
            return {
                "success": False,
                "message": "This rental bill is already finalized.",
            }

        if self.status == "cancelled":
            return {
                "success": False,
                "message": "Cancelled rental bill cannot be finalized.",
            }
        ...
        if not weighted_rows:
            return {
                "success": False,
                "message": "No active investor found for this rental month.",
            }
        ...
        if total_weight <= 0:
            return {
                "success": False,
                "message": "Total rental weight is zero.",
            }

        with transaction.atomic():
            self.distribution_lines.all().delete()

            user_balance_changes = defaultdict(lambda: Decimal("0.00"))
            total_management_deduction = Decimal("0.00")
            distributed_total = Decimal("0.00")
            total_rows = len(weighted_rows)
            ...
            for index, row in enumerate(weighted_rows):
                ...
                if net_amount > 0:
                    ...
                    user_balance_changes[user.id] += final_profit
                    total_management_deduction += deduction
                    ...
                    user_balance_changes[user.id] -= loss_amount

                RentalBillDistribution.objects.create(
                    rental_bill=self,
                    property_contribution=contrib,
                    user=user,
                    contribution_amount=Decimal(str(contrib.contribution or 0)),
                    month_days=row["month_days"],
                    days_proportion=row["days_proportion"],
                    shares=row["shares"],
                    base_weight=row["base_weight"],
                    buyer_level=row["buyer_level"],
                    buyer_multiplier=row["buyer_multiplier"],
            ...
            for user_id, amount in user_balance_changes.items():
                user = User.objects.select_for_update().get(id=user_id)
                old_balance = Decimal(str(user.balance or 0))
                user.balance = (
                    old_balance + amount
            ...
            if net_amount > 0 and total_management_deduction > 0:
                office_manager = User.get_office_manager()
                office_cost, _ = OfficeCost.objects.get_or_create(id=1)

                if office_manager:
                    office_manager.balance = (
                        Decimal(str(office_manager.balance or 0)) + total_management_deduction
                    ).quantize(Decimal("0.000001"), rounding=ROUND_HALF_UP)
                    office_manager.save(update_fields=["balance"])
                else:
                    office_cost.balance = (
                        Decimal(str(office_cost.balance or 0)) + total_management_deduction
                    ).quantize(Decimal("0.000001"), rounding=ROUND_HALF_UP)
                    office_cost.save(update_fields=["balance"])
            ...
            self.save(update_fields=["status", "finalized_at", "finalized_by"])

        return {
            "success": True,
            "message": "Rental bill finalized successfully.",
            "net_amount": net_amount,
            "total_management_deduction": total_management_deduction,
        }
...
class RentalExpense(models.Model):
    ...
    receipt = models.FileField(
        upload_to="rental_expenses/receipts/",
        null=True,
        blank=True,
    ...
    paid_by_user = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="paid_rental_expenses",
        verbose_name="Paid By User",
    )
    ...
    created_by = models.ForeignKey(
        "User",
        on_delete=models.CASCADE,
        related_name="created_rental_expenses",
    ...
    approved_by = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="approved_rental_expenses",
        verbose_name="Approved By",
    )
    ...
    def __str__(self):
        return f"{self.rental_bill} - {self.purpose} - ${self.amount}"

    def clean(self):
        if self.amount is not None and self.amount < 0:
            raise ValidationError("Expense amount cannot be negative.")

        if self.rental_bill and self.rental_bill.status == "finalized":
            raise ValidationError("Cannot add or change expense after rental bill is finalized.")

    def approve(self, approved_by):
        if not approved_by.is_superuser:
            return False, "Only superuser can approve expense."

        if self.status != "pending":
            return False, "Only pending expense can be approved."

        if self.rental_bill.status == "finalized":
            return False, "Cannot approve expense after rental bill is finalized."

        self.status = "approved"
        self.approved_by = approved_by
        self.approved_at = timezone.now()
        self.save(update_fields=["status", "approved_by", "approved_at"])
        return True, "Rental expense approved successfully."

    def reject(self, rejected_by):
        if not rejected_by.is_superuser:
            return False, "Only superuser can reject expense."

        if self.status != "pending":
            return False, "Only pending expense can be rejected."

        if self.rental_bill.status == "finalized":
            return False, "Cannot reject expense after rental bill is finalized."

        self.status = "rejected"
        self.approved_by = rejected_by
        self.approved_at = timezone.now()
        self.save(update_fields=["status", "approved_by", "approved_at"])
        return True, "Rental expense rejected successfully."


class RentalBillDistribution(models.Model):
    rental_bill = models.ForeignKey(
        "RentalBill",
        on_delete=models.CASCADE,
    ...
    contribution_amount = models.DecimalField(max_digits=50, decimal_places=2, default=Decimal("0.00"))
    month_days = models.PositiveIntegerField(default=0)
    days_proportion = models.DecimalField(max_digits=20, decimal_places=6, default=Decimal("0.00"))
    shares = models.DecimalField(max_digits=20, decimal_places=6, default=Decimal("0.00"))
    base_weight = models.DecimalField(max_digits=20, decimal_places=6, default=Decimal("0.00"))

    buyer_level = models.CharField(max_length=20, null=True, blank=True)
    buyer_multiplier = models.DecimalField(max_digits=10, decimal_places=2, default=Decimal("1.00"))
    final_weight = models.DecimalField(max_digits=20, decimal_places=6, default=Decimal("0.00"))

    gross_profit = models.DecimalField(max_digits=50, decimal_places=2, default=Decimal("0.00"))
    deduction_percentage = models.DecimalField(max_digits=10, decimal_places=2, default=Decimal("0.00"))
    deduction = models.DecimalField(max_digits=50, decimal_places=2, default=Decimal("0.00"))
    final_profit = models.DecimalField(max_digits=50, decimal_places=2, default=Decimal("0.00"))
    loss_amount = models.DecimalField(max_digits=50, decimal_places=2, default=Decimal("0.00"))
