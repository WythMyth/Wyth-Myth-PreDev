...
from .models import RentalBillDistribution
@login_required
def dashboard(request):
    summary = calculate_user_investment_summary(request.user)
    ...
    purchased_properties = Property.objects.filter(
        status__in=["bought", "ready_to_sell", "pending", "sold", "rented"]
    )
    ...
    rental_distribution_qs = (
        RentalBillDistribution.objects
        .select_related(
            "rental_bill",
            "rental_bill__property",
            "property_contribution",
        )
        .filter(
            user=request.user,
            rental_bill__status="finalized",
        )
        .order_by(
            "-rental_bill__bill_month",
            "rental_bill__property__property_name",
            "property_contribution__investment_sequence",
            "id",
        )
    )
    ...
    for row in rental_distribution_qs:
        rental_income_rows.append({
            "property": row.rental_bill.property,
            "bill": row.rental_bill,
            "month": row.rental_bill.bill_month,
            "rent_amount": row.rental_bill.rent_amount,
            "total_expense": row.rental_bill.total_expense,
            "net_amount": row.rental_bill.net_amount,
    ...
    rental_income_total_gross = rental_distribution_qs.aggregate(
        total=Coalesce(Sum("gross_profit"), Decimal("0.00"))
    )["total"]

    rental_income_total_deduction = rental_distribution_qs.aggregate(
        total=Coalesce(Sum("deduction"), Decimal("0.00"))
    )["total"]

    rental_income_total_final_profit = rental_distribution_qs.aggregate(
        total=Coalesce(Sum("final_profit"), Decimal("0.00"))
    )["total"]

    rental_income_total_loss = rental_distribution_qs.aggregate(
        total=Coalesce(Sum("loss_amount"), Decimal("0.00"))
    )["total"]
    ...
    context = {
        ...
        "rental_income_total_gross": rental_income_total_gross,
        "rental_income_total_deduction": rental_income_total_deduction,
        "rental_income_total_final_profit": rental_income_total_final_profit,
        "rental_income_total_loss": rental_income_total_loss,
    }
...
class RentalBillListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        queryset = (
            RentalBill.objects
            .select_related("property", "approved_by", "finalized_by")
            .prefetch_related("expenses")
            .order_by("-bill_month", "-created_at")
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        context["total_rent"] = queryset.aggregate(total=Sum("rent_amount"))["total"] or 0
        context["total_expense"] = sum(bill.total_expense for bill in queryset)
        context["total_net"] = sum(bill.net_amount for bill in queryset)
...
class RentalBillCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        messages.success(self.request, "Rental bill created successfully. Superuser approval is required before final distribution.")
        return super().form_valid(form)
...
class RentalBillUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if self.object.status in ["approved", "finalized"]:
            messages.error(request, "Approved or finalized rental bill cannot be changed.")
            return redirect("accounts:rental_bill_detail", pk=self.object.pk)
...
class RentalBillDeleteView(LoginRequiredMixin, DeleteView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if self.object.status in ["approved", "finalized"]:
            messages.error(request, "Approved or finalized rental bill cannot be deleted.")
            return redirect("accounts:rental_bill_detail", pk=self.object.pk)
...
def approve_rental_bill(request, pk):
    rental_bill = get_object_or_404(RentalBill, pk=pk)
    ...
    success, message = rental_bill.approve(request.user)
...
class RentalExpenseCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        form.instance.created_by = self.request.user
        messages.success(self.request, "Rental expense created successfully. Superuser approval is required.")
        return super().form_valid(form)
...
def approve_rental_expense(request, pk):
    ...
    success, message = expense.approve(request.user)
...
def reject_rental_expense(request, pk):
    ...
    success, message = expense.reject(request.user)
